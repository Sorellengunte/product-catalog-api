...
const ProductDetailPage: React.FC = () => {
  ...
  const loadProduct = async () => {
    ...
    try {
      ...
      if (data) {
        // On s'assure que "images" existe toujours
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Notification */}
      {showNotification && (
        <div className="fixed top-4 right-4 z-50 bg-green-50 border border-green-200 text-green-800 rounded-xl p-4 shadow-lg transform transition-all duration-300 animate-fade-in">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex gap-3 overflow-x-auto">
              {product.images.map((img, i) => (
                <button
                  ...
                  className={`w-20 h-20 border-2 rounded ${selectedImage === i ? 'border-blue-500' : 'border-gray-200'}`}
                >
          ...
          <div className="lg:w-1/2 bg-white rounded-2xl shadow-lg p-6 flex flex-col gap-4">
            ...
            <div className="flex items-center gap-3">
              {[...Array(5)].map((_, i) => (
                <Star
                  key={i}
                  className={`w-5 h-5 ${
                    i < Math.floor(product.rating) ? 'fill-amber-500 text-amber-500' : 'fill-gray-200 text-gray-200'
                  }`}
                />
              ))}
            ...
            <button
              ...
              className={`py-4 rounded-xl font-bold text-white ${product.stock === 0 ? 'bg-gray-300' : 'bg-blue-600 hover:bg-blue-700'}`}
            >
              <ShoppingBag className="inline w-5 h-5 mr-2" />
              {product.stock === 0 ? 'Rupture' : 'Ajouter au panier'}
            </button>

            <button onClick={() => navigate('/products')} className="text-blue-600 mt-4 flex items-center gap-2">
